...
data:extend{{type = "item-subgroup", name = "ingots-blast-furnace", group = "intermediate-products", order = "c2a"}}
...
for_all_processable(
    function(name, values)
        SODA.recipe.add_simple(
            name .. "-ingot-blast-furnace", "blast-smelting", {{"activated-black", 2}, {"stone", 2}, {name .. "-ore", 4}}, nil, {{name .. "-ingot", 1}, {"slag", 1}}, nil,
            SODA.constants.processing.total_time_per_ingot[2], "ingots-blast-furnace", values.order
        )
        SODA.icon.make("recipe", name .. "-ingot-blast-furnace", 64, SODA.path.icons("processing/ingot"), nil, values.tint)
    end
)
SODA.item.add("slag", "a", "side-products", 50, {folders = "processing"})
SODA.recipe.add_simple("slag-crushing", "crushing", "slag", 1, "stone", 2, SODA.constants.processing.total_time_per_ingot[2] / 4)
...
SODA.recipe.add_simple("yellow-reduction", "smelting", "yellow-oxide", 4, "yellow-ore", 3, SODA.constants.processing.total_time_per_ingot[2] / 2)
...
for_all_processable(
    function(name, values)
        SODA.item.add(name .. "-purified", values.order, "purified", 100, {folders = "processing", name = "purified", tint = values.tint})
        SODA.recipe.add_simple(
            name .. "-purified", "chemistry", {{name .. "-ore", 4}, {type = "fluid", name = "green-acid", amount = 50}}, nil, name .. "-purified", 4, SODA.constants.processing.total_time_per_ingot[2],
            nil, nil, values.tint
        )
        SODA.recipe.add_simple(name .. "-ingot-purified", "smelting", name .. "-purified", 4, name .. "-ingot", 1, SODA.constants.processing.total_time_per_ingot[2], "ingots-purified", values.order)
        SODA.icon.make("recipe", name .. "-ingot-purified", 64, SODA.path.icons("processing/ingot"), nil, values.tint)
    end
)

-- MID (0.5 fuel per ingot)
